...
function SpamMessages(amount: number, channel: Channel, content: string) {
    let message = {
        // The following are required to prevent Discord from throwing an error (also this plugin is 100% skidded)
        invalidEmojis: [],
        tts: false,
...
export default definePlugin({
    ...
    authors: [{name: "Goober", id: 1288185194926575717n}],
    options: {
        baddies: {
            name: "Spam plugin",
            description: "Just a little spam plugin for pookies.",
            ...
            async execute(args, ctx) {
                switch (args[0].name) {
                    case "spam": {
                        ...
                        const msg: string = findOption(args[0].options, "content", "Nova likes minors!");
